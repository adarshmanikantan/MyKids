...
public class EnglishAdapter extends RecyclerView.Adapter<EnglishAdapter.MyViewHolder> {
    ...
    public EnglishAdapter(Context context, EnglishModel englishModel)
    {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, final int position) {
        Glide.with(context)
                ...
                .load(englishModel.getVideos().get(position).getImage().trim()) // Image URL
...
//                .error(R.drawable.ic_error_black_36dp) // On error image
        ...
        holder.title.setText(englishModel.getVideos().get(position).getTitle());
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SharedPreferences sharedPreferences=context.getSharedPreferences("pref",Context.MODE_PRIVATE);
                SharedPreferences.Editor editor=sharedPreferences.edit();
                editor.putString("url",englishModel.getVideos().get(position).getVideo());
                editor.apply();
    ...
    public int getItemCount() {
        return englishModel.getVideos().size();
    }
