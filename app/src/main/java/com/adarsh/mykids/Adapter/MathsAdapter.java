...
public class MathsAdapter extends RecyclerView.Adapter<MathsAdapter.MyViewHolder> {
    Context context;
    MathsModel mathsModel;
    public MathsAdapter(Context context, MathsModel mathsModel)
    {
        this.context=context;
        this.mathsModel=mathsModel;
    }
    @NonNull
    @Override
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, final int position) {
        Glide.with(context)
                ...
                .load(mathsModel.getVideos().get(position).getImage().trim()) // Image URL
...
//                .error(R.drawable.ic_error_black_36dp) // On error image
        ...
        holder.title.setText(mathsModel.getVideos().get(position).getTitle());
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SharedPreferences sharedPreferences=context.getSharedPreferences("pref",Context.MODE_PRIVATE);
                SharedPreferences.Editor editor=sharedPreferences.edit();
                editor.putString("url",mathsModel.getVideos().get(position).getVideo());
                editor.apply();
    ...
    public int getItemCount() {
        return mathsModel.getVideos().size();
    }
